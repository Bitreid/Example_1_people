...
class People
{
    ...
    public function __construct($db, $id = '', $name = '', $surname = '',
      ...
      $this->conn = $db;
      //либо берет информацию по id
      if ($id!='') {
        $this->readOne($id);
      }
      //либо создает человека в БД с заданной информацией
      else if ($name != ''){
        $this->name = $name;
    ...
    function create()
    {
      ...
      if ($stmt->execute()) {
          echo "<div class='alert alert-success'>Информация успешно добавленя.</div>";
        } else {
